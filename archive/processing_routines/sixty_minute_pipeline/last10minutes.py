...
def getAllInteractSummary():
    ...
    for request in all_interac_requests:
        ...
        event_list = list(db._client['production']['eventCollection'].find({
            'created': {
                '$lt': request['created'], 
                '$gte': request['created']-datetime.timedelta(seconds=10*60)
                },
            '$or': [{'metadata.email': email},
    ...
    records = records[records.email != 'placeholder'] # to get rid of the blank row
    
    return records

def getLastInteractSummary(email, nth=0):
    # connect to the database
    db = junodb.Database(creds)
    ...
    latest_interac_request = list(db._client['production']['eventCollection'].find({'eventCategory': 'interac', 'eventAction': 'request','$or': [{'metadata.email': email},
                {'metadata.prossessorResponse.email': email},
                {'metadata.prossessorResponse.profile.email': email},
                {'metadata.requestParams.email': email}]}).sort('created', -1).limit(10))

    latest_interac_request = latest_interac_request[nth]

    # get the events directly preceeding the transaction
    event_list = list(db._client['production']['eventCollection'].find({
        'created': {
            '$lt': latest_interac_request['created'],
            '$gte': latest_interac_request['created']-datetime.timedelta(seconds=10*60)
            }, 
        '$or': [{'metadata.email': email},
                {'metadata.prossessorResponse.email': email},
                {'metadata.prossessorResponse.profile.email': email},

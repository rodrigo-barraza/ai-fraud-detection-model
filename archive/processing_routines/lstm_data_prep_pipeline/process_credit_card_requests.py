...
# get all of the credit_card requests
credit_card_requests = all_events[(all_events['metadata.email'].isnull() == False) & (all_events.eventCategory == 'buy') & (all_events.eventAction == 'request')].reset_index(drop=True)
...
credit_card_requests = removeWhitelistRecords(flagFraudsters(credit_card_requests))

print("credit_card Requests Shape:", credit_card_requests.shape)

result = credit_card_requests[['_id','metadata.email','created','metadata.amount','metadata.rate','value']]
...
# convert the credit_card results to a dict
...
# hacky way to number the sequence events prior to each credit_card request with 0 being the request and 10 being the 10th event prior to the request
...
fraud = credit_card_requests.fraud
emails = credit_card_requests['metadata.email']
...
# print the number of credit_card requests per fraudulent user
...
with open("results/lstm_credit_card_results.pickle", 'wb') as outfile:
